...
class _DepthBuffer {
    private:
        ...
        void ensureMiddleX(cl_float3 &v1, cl_float3 &v2, cl_float3 &v3) {
            if (v1.s[0] > v2.s[0]) std::swap(v1, v2);
            if (v1.s[0] > v3.s[0]) std::swap(v1, v3);
            if (v2.s[0] > v3.s[0]) std::swap(v2, v3);
            std::swap(v1, v2);
        }
        ...
        void initOpenCL() {
            ...
            std::string sourceCode = getCode("./cl_scripts/main/script6.cl");
            sources.push_back({sourceCode.c_str(), sourceCode.length()});

            cl::Program program(getGPU().getContext(), sources);
            program.build("-cl-std=CL3.0 -cl-mad-enable -cl-unsafe-math-optimizations");
        ...
        /*    cl_float3 v1 = {{x1, y1, 1.0f/z1}};
            ...
            boxLeft = std::max(boxLeft,-maxx/2);
            boxRight = std::min(boxRight,maxx/2);
            boxTop = std::max(boxTop,-maxy/2);
            boxBottom = std::min(boxBottom,maxy/2);
            if(boxLeft>=boxRight || boxTop>=boxBottom){
                return;
            }
           
            assert(drawingKernel->setArg(5, int(boxLeft))==CL_SUCCESS); 
            assert(drawingKernel->setArg(6, int(boxTop))==CL_SUCCESS); 
            assert(drawingKernel->setArg(7, int(maxx/2))==CL_SUCCESS); 
            assert(drawingKernel->setArg(8, int(maxy/2))==CL_SUCCESS); 
            assert(drawingKernel->setArg(9, clr)==CL_SUCCESS); 

            float inv = 1.0f/(x1*y2 - x1*y3 - y1*x2 + y1*x3 + x2*y3 - y2*x3);
            //assert(drawingKernel->setArg(10, inv)==CL_SUCCESS); 
            cl_float3 du = {{(-x1*y3 + y1*x3)*inv,(y3 - y1)*inv,(x1 - x3)*inv}};
            cl_float3 dv = {{(x1*y2 - y1*x2)*inv,(y1 - y2)*inv,(x2 - x1)*inv}};
            assert(drawingKernel->setArg(10, (-x1*y3 + y1*x3)*inv)==CL_SUCCESS); 
            assert(drawingKernel->setArg(11, (x1*y2 - y1*x2)*inv)==CL_SUCCESS); 
            assert(drawingKernel->setArg(12, (y3 - y1)*inv)==CL_SUCCESS); 
            assert(drawingKernel->setArg(13, (x1 - x3)*inv)==CL_SUCCESS); 
            assert(drawingKernel->setArg(14, (y1 - y2)*inv)==CL_SUCCESS); 
            assert(drawingKernel->setArg(15, (x2 - x1)*inv)==CL_SUCCESS); 

            //assert(drawingKernel->setArg(10, inv)==CL_SUCCESS); 
            
            size_t global_work_size_x = (boxRight - boxLeft +  6)/6;
            size_t global_work_size_y = (boxBottom - boxTop + 1)/1;
            cl::NDRange global_work_size(global_work_size_x, global_work_size_y);
            assert(monitorExecution(getGPU().getQueue().enqueueNDRangeKernel(*drawingKernel, cl::NullRange, global_work_size, cl::NullRange)));
